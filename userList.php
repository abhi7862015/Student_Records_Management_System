...
if (isset($_GET['user_id']) && $_GET['user_id'] != 0) {
    ...
    if (isset($_GET['account']) && $_GET['account'] == "disabled") {
        ...
        if ($conn->affected_rows) {
            if ($UserType == "Student") {
                $query3 = "Update student_details set user_id=0 where user_id=" . $user_id;
            } else if ($UserType == "Teacher") {
                $query3 = "Update teacher_details set user_id=0 where user_id=" . $user_id;
            }
            $conn->query($query3);
...
$query1 = "select * from user_details " . $all_filter . $search_status . $search_type . $status_type . $search_cond . $status_cond . $type_cond;
